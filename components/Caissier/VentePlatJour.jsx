...
const VentePlatJour = () => {
  ...
  const [montantGlobal_jour, setmontantGlobal_jour] = useState(0);
  ...
  const { currentUser } = useContext(AuthContext)
        ...
        useEffect(() => {
          if (userid) {
            axios.get(`${StaticIP}api/vente/ventes-plat-du-jour-caissier/${userid}`)
              .then(response => {
                setVentes(response.data.ventes);
                setMontantGlobal(response.data.montantGlobal);
                setmontantGlobal_jour(response.data.montantGlobal_jour);
              })
              .catch(error => {
  ...
  return (
    <div className="bg-white shadow-md rounded-lg">
      ...
      <div className="card">
        ...
        <tfoot>
          <tr className="bg-gray-200 font-bold">
            <td className="border p-2 text-right" colSpan="2"><b>Total des ventes du jour : Plats + Boissons </b></td>
            <td className="border p-2 text-center" style={{color:'red'}}><b>{montantGlobal_jour.toLocaleString()}</b></td>
          </tr>
        </tfoot>
      </table>
          </div>
