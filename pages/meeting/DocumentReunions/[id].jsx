...
const DocumentReunions = () => {
    ...
    const fetchdoc = async () => {
        ...
        try {
            const response = await axios.get(`${StaticIP}api/documentreunion/liste?id=${id}`);
    ...
    useEffect(() => {
        fetchdoc();
        setdataDocument((prev) => ({ ...prev, reunion_id: id }));
    }, [id]);
    ...
    const handleSubmit = async (e) => {
        ...
        formData.append('reunion_id', dataDocument.reunion_id);
        ...
        setLoading(true);
        try {
            const response = await axios.post(`${StaticIP}api/documentreunion/nouveau`, formData, {
    ...
    const handleDelete = async () => {
        ...
        try {
            const response = await axios.delete(`${StaticIP}api/documentreunion/supprimer/${selectedDocument.id}`);
            if (response.data.Status) {
  ...
  return (
    <LayoutMeeting>
            <BreadCrumb titre="Documents de la réunion" />
                                            ...
                                            <table id="myDataTable" class="table table-hover align-middle mb-0" style={{ width:'100%'}}>
                                                ...
                                                {doc.map((doc, index) => (
                                                    <tr key={doc.id}>
                                                        <td>{index +1}</td>
                                                        <td>
                                                            <a href={`/public/Reunions/${doc.fichier}`} target="_blank" >
                                                                {doc.fichier}
                                                            </a>
                                                        </td>
...
export default DocumentReunions